...
本章では、`List`や`Option`、その他多種多様な文脈の中における計算の連なりを表現することを可能にする抽象概念である、 **ファンクタ** を調べていく。
...
`map`を、リストを走査するものとしてではなく、一度に中身のすべてを変換するものだと考えてみよう。
...
`map`は、反復処理のパターンではなく、関連するデータ型によって規定される何らかの複雑な文脈を無視した、中身の値に対する計算を繋げるための方法だと考えたほうがよい:
...
`Future`はこのような形で、`List`、`Option`、そして`Either`の例で見たような、計算の連鎖という振る舞いを提供している。
...
Scala の`Future`は **参照透過** ではないので、純粋関数プログラミングの良い例ではないということに注意してほしい。
`Future`は計算の結果を常にキャッシュする。この振る舞いを改変する方法はない。
`Future`を「副作用を持つ計算」を包むのに利用すると、想定外の結果が発生することになる。
例えば:
...
このような食い違いが、`Future`と副作用を含むプログラムの意味を考えるのを難しくする。
`Future`の振る舞いには他にも問題点がある。ユーザがプログラムを実行する時点を指定できず、常に計算が即座に開始するという振る舞いが問題である。
詳しくは Rob Norrisによる[素晴らしい Reddit の投稿][link-so-future]を参照のこと。
</div>

`Future`は参照透過ではないが、よく似たデータ型で参照透過なものがある。
お分かりいただけるだろうか…
...
```tut:book
(func1 map func2)(1)     // map を利用した合成
(func1 andThen func2)(1) // andThen を利用した合成
func2(func1(1))          // 手動の合成
```
...
[@sec:functors:partial-unification]節で、なぜこのようなことが起きるのかについて詳しく見ていく。
</div>
...
`map`メソッドは最初の`F[A]`を変換関数とは別の引数として受け取る。
簡略化した定義は以下の通りである:
...
`F[_]`のような構文をここで初めて見る読者もいるだろう。**型コンストラクタ** と **高カインド型** について考えるために、少し回り道をしよう。
`scala.language`インポートについても説明する。

<div class="callout callout-warning">
**ファンクタの法則**

ファンクタは、多くの小さな計算を1つずつ連鎖しても、`map`を行う前にそれらを1つの大きな関数として組み合わせたとしても、意味が等しくなることを保証する。
このことを保証するには、次の法則を満たさなければならない:

**恒等関数の法則**: 恒等関数を引数に`map`を呼び出した結果は、何もしなかった場合の結果と同じでなければならない:
...
これは関数と値との間の関係に似ている。
関数は、引数を与えることで値を生み出す「値コンストラクタ」と考えることができる:
...
これは関数の定義においては引数を指定しなければならない一方で、それを参照する際は引数を省略するのに似ている:
...
型コンストラクタについての知識があれば、Cats における`Functor`の定義は、`List`、`Option`、`Future`、または型エイリアス`MyFunc`のような、1つの型パラメータをとるすべての型コンストラクタに対して、`Functor`のインスタンスを作れるということを表しているのが分かるだろう。

<div class="callout callout-info">
**言語機能(language feature)のインポート**

Scala では、高カインド型は発展的な言語機能とみなされている。
`A[_]`という構文で型コンストラクタを宣言する際は、高カインドの言語機能を「有効」にし、コンパイラの警告を抑制する必要がある。

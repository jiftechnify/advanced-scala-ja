...
形式的にいえば、`A`という型に対するモノイドは、以下の要素を持つものだ:
...
半群(semigroup)は、単に「モノイドの`combine`の部分」だということができる。
多くの半群はモノイドでもあるが、`empty`要素を定義できないようなデータ型もある。
...
`A`型に対する`Monoid`を定義すれば、`Semigroup`がタダで手に入る。
同じように、`Semigroup[B]`型の引数を要求するメソッドに対して`Monoid[B]`を代わりに渡すこともできる。
